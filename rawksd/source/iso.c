...
static int _read(void *ptr, u64 offset, size_t len) {
    ...
    if (WDVD_DVDRead(read_buffer, BUFFER_SIZE / SECTOR_SIZE, sector)) {
        last_access = gettime();
        cache_sectors = 0;
        return -1;
    }
...
static VOLUME_DESCRIPTOR *read_volume_descriptor(u8 descriptor) {
    ...
    for (sector = 16; sector < 32; sector++) {
        if (WDVD_DVDRead(read_buffer, 1, sector)) return NULL;
...
static bool read_directories() {
	VOLUME_DESCRIPTOR *volume = NULL;
    volume = read_volume_descriptor(2);
    if (volume) unicode = true;
    else
    if (!(volume = read_volume_descriptor(1))) return false;
    ...
    while (i < 0xffff && offset < path_table_len) {
        ...
        int to_read = MIN(sizeof(PATHTABLE_ENTRY), path_table_len-offset);
        if (_read(&entry, (u64)path_table * SECTOR_SIZE + offset, to_read) != to_read) return false; // kinda dodgy - could be reading too far
...
bool ISO9660_Mount() {
    ...
    ISO9660_Unmount();
    if (WDVD_Init()<0 || WDVD_SetDVDMode(1)<0)
    	return false;
